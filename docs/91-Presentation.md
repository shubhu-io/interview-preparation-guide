...
```mermaid
graph TD
    A[Opening<br/>Hook + Context + Agenda] --> B[Body<br/>Point 1 + Evidence]
    B --> C[Point 2 + Evidence]
    C --> D[Point 3 + Evidence]
    D --> E[Closing<br/>Summary + CTA + Ending]
```

### Q&A Response Flow
```mermaid
graph TD
    A[Listen to question] --> B[Repeat question]
    B --> C{Know answer?}
    C -->|Yes| D[Answer concisely]
    C -->|No| E[Honest + follow up]
    D --> F[Bridge to key message]
    E --> F
    F --> G[Check if addressed concern]
```

### Slide Design Principles
```mermaid
graph LR
    A[One Idea Per Slide] --> B[Minimal Text]
    B --> C[Large Font 30pt+]
    C --> D[High Quality Visuals]
    D --> E[Consistent Design]
```
