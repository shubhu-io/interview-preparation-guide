...
```mermaid
flowchart TD
    A[Team Fit Assessment] --> B[Collaboration Style]
    A --> C[Conflict Resolution]
    A --> D[Communication Skills]
    A --> E[Adaptability]
    B --> F[Interview Evaluation]
    C --> F
    D --> F
    E --> F
    F --> G{Team Fit Score}
    G -->|Strong| H[Team Integration]
    G -->|Moderate| I[Development Plan]
    G -->|Weak| J[Alternative Team]
```

### Tuckman's Stages with Your Role
```mermaid
flowchart LR
    A[Forming<br/>Be welcoming] --> B[Storming<br/>Navigate conflict]
    B --> C[Norming<br/>Establish norms]
    C --> D[Performing<br/>High contribution]
    D --> E[Adjourning<br/>Graceful close]
```
