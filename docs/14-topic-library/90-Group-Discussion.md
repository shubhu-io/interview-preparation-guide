...
nav_order: 90
permalink: /14-topic-library/90-Group-Discussion/
---

<div class="topic-meta">
  <span class="topic-badge">Topic #90</span>
...
```mermaid
graph TD
    A[Listen carefully] --> B[Formulate point]
    B --> C[Wait for appropriate moment]
    C --> D[Make structured contribution]
    D --> E[Listen to responses]
    E --> F{More to add?}
    F -->|Yes| B
    F -->|No| G[Support others' points]
```

### GD Time Management
```mermaid
graph LR
    A[0-2 min<br/>Listen & Form] --> B[2-8 min<br/>Make 2-3 points]
    B --> C[8-15 min<br/>Build & Introduce]
    C --> D[15-20 min<br/>Summarize]
```

### Argument Structure (PIE)
```mermaid
graph TD
    A[Point] -->|State clearly| B[Illustration]
    B -->|Provide example| C[Explanation]
    C -->|Connect to topic| D[Conclusion]
```
