...
nav_order: 85
permalink: /14-topic-library/85-HR-Interview/
---

<div class="topic-meta">
  <span class="topic-badge">Topic #85</span>
...
```mermaid
graph LR
    A[Phone Screen] --> B[Technical Rounds]
    B --> C[HR Interview]
    C --> D{Decision}
    D -->|Hire| E[Offer]
    D -->|No Hire| F[Rejection]
    E --> G[Negotiation]
    G --> H[Accept]
```
...
```mermaid
graph TD
    A[Situation] -->|Set context| B[Task]
    B -->|Define challenge| C[Action]
    C -->|Your specific steps| D[Result]
    D -->|Quantifiable outcome| E[Impact]
```

### Salary Negotiation Flow
```mermaid
graph TD
    A[Research Market Rate] --> B[Let Them Go First]
    B --> C[Counter with Data]
    C --> D[Negotiate Total Package]
    D --> E{Agreement?}
    E -->|Yes| F[Get in Writing]
    E -->|No| G[Consider Alternatives]
    G --> H{Acceptable?}
    H -->|Yes| F
    H -->|No| I[Politely Decline]
```
