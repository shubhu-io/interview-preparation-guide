...
nav_order: 11
permalink: /14-topic-library/11-Verbal-Ability/
---

<div class="topic-meta">
  <span class="topic-badge">Topic #11</span>
...
```mermaid
graph TD
    A[Assess Current Level] --> B[Build Vocabulary]
    B --> C[Master Grammar]
    C --> D[Practice Reading]
    D --> E[Learn Comprehension]
    E --> F[Sentence Correction]
    F --> G[Para Jumbles]
    G --> H[Mock Tests]
...
```mermaid
graph TD
    A[Verbal Ability] --> B[Grammar]
    A --> C[Vocabulary]
    A --> D[Reading]
    A --> E[Reasoning]
    
    B --> B1[Parts of Speech]
    B --> B2[Tenses]
    B --> B3[Agreement]
...
```mermaid
flowchart LR
    A[Reading Task] --> B{Purpose?}
    B -->|Main Ideas| C[Skim]
    B -->|Specific Info| D[Scan]
    B -->|Deep Understanding| E[Active Reading]
    
    C --> C1[Read title/headings]
    C --> C2[First/last sentences]
    C --> C3[Key words]
    
    D --> D1[Look for keywords]
    D --> D2[Read context]
    D --> D3[Find exact answers]
    
    E --> E1[Underline key points]
    E --> E2[Ask questions]
    E --> E3[Make connections]
```
...
```mermaid
flowchart TD
    A[Read Sentence] --> B[Identify Subject/Verb]
    B --> C{Agreement OK?}
    C -->|No| D[Fix Agreement]
    C -->|Yes| E{Tense Consistent?}
    D --> E
    E -->|No| F[Fix Tense]
    E -->|Yes| G{Parallel Structure?}
    F --> G
    G -->|No| H[Fix Parallel]
    G -->|Yes| I{Modifiers OK?}
    H --> I
    I -->|No| J[Fix Modifiers]
    I -->|Yes| K[Sentence OK]
    J --> K
```
...
```mermaid
graph TD
    A[New Word] --> B[Learn Meaning]
    B --> C[Study Root/Parts]
    C --> D[Find Synonyms]
    D --> E[Use in Context]
    E --> F[Practice Daily]
    F --> G[Master Word]
