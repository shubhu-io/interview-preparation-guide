...
```mermaid
flowchart TD
    Start[Start Here] --> T1[Tier 1: Must-Have]
    T1 --> T2[Tier 2: Recommended]
    T2 --> T3[Tier 3: Nice to Have]
    
    T1 --> Books1[CTCI, System Design Interview]
    T1 --> Websites1[LeetCode, Pramp]
    T1 --> Courses1[Grokking System Design]
    
    T2 --> Books2[DDIA, STAR Interview]
    T2 --> Websites2[HackerRank, Blind]
    T2 --> Courses2[Coursera, Udemy]
    
    T3 --> Books3[Manager's Path]
    T3 --> Websites3[Stack Overflow]
    T3 --> Tools3[Anki, Notion]
```
...
```mermaid
flowchart LR
    A[Foundation] --> B[Deepening]
    B --> C[Advanced]
    C --> D[Interview Ready]
    
    A --> A1[CTCI + LeetCode Easy]
    B --> B1[DDIA + System Design]
    C --> C1[Company-specific + Mocks]
    D --> D1[Final Review + Rest]
```
