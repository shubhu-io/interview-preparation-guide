...
nav_order: 88
permalink: /14-topic-library/88-Leadership/
---

<div class="topic-meta">
  <span class="topic-badge">Topic #88</span>
...
```mermaid
graph TD
    A[Team Member Maturity] --> B{Low Competence?}
    B -->|Yes| C[Directing]
    B -->|No| D{Variable Commitment?}
    D -->|Yes| E[Coaching]
    D -->|No| F{High Competence?}
    F -->|Yes| G[Delegating]
    F -->|No| H[Supporting]
```
...
graph TD
    ...
    B -->|Yes: Two-way door| C[Decide quickly]
    C --> D[Execute]
    D --> E[Learn from outcome]
    E --> F[Adjust if needed]
    B -->|No: One-way door| G[Gather more data]
    G --> H[Analyze thoroughly]
    H --> I[Make careful decision]
    I --> J[Execute with commitment]
```
