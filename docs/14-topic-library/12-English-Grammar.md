...
```mermaid
graph TD
    A[English Grammar Fundamentals] --> B[Tenses]
    A --> C[Parts of Speech]
    A --> D[Sentence Structure]
    B --> B1[Simple Tenses]
    B --> B2[Perfect Tenses]
    B --> B3[Continuous Tenses]
    B --> B4[Perfect Continuous]
    C --> C1[Nouns & Pronouns]
    C --> C2[Verbs & Adverbs]
    C --> C3[Adjectives & Articles]
    C --> C4[Prepositions & Conjunctions]
    D --> D1[Subject-Verb Agreement]
    D --> D2[Modifiers]
    D --> D3[Parallelism]
    D --> D4[Active & Passive Voice]
    D --> D5[Direct & Indirect Speech]
    E[Advanced Grammar] --> E1[Common Errors]
    E --> E2[Professional Writing]
    E --> E3[Formal vs Informal]
    F[Interview Application] --> F1[Resume Grammar]
    F --> F2[Email Etiquette]
    F --> F3[Verbal Communication]
```
...
```mermaid
flowchart TD
    A[When did the action happen?] --> B{Past or Present or Future?}
    B -->|Past| C{Completed?}
    C -->|Yes| D[Simple Past or Past Perfect]
    C -->|No| E[Past Continuous]
    B -->|Present| F{Ongoing or Habitual?}
    F -->|Habitual| G[Simple Present]
    F -->|Ongoing| H[Present Continuous]
    F -->|Started in past, continues now| I[Present Perfect Continuous]
    B -->|Future| J{Completed before a point?}
    J -->|Yes| K[Future Perfect]
    J -->|No| L[Simple Future or Future Continuous]
```

### Diagram 3: Active vs Passive Voice Decision
```mermaid
flowchart TD
    A[Writing a sentence] --> B{Is the doer important?}
    B -->|Yes| C[Use Active Voice]
    B -->|No| D{Is the doer known?}
    D -->|No| E[Use Passive Voice]
    D -->|Yes| F{Is the receiver more important?}
    F -->|Yes| E
    F -->|No| C
    E --> G[Object + was/were + V3 + by subject]
    C --> H[Subject + V + Object]
```

### Diagram 4: Common Error Detection Process
```mermaid
flowchart TD
    A[Review Sentence] --> B{Subject-Verb Agreement OK?}
    B -->|No| C[Fix number agreement]
    B -->|Yes| D{Tense Consistent?}
    D -->|No| E[Fix tense sequence]
    D -->|Yes| F{Modifier Placement OK?}
    F -->|No| G[Move modifier next to target word]
    F -->|Yes| H{Parallel Structure?}
    H -->|No| I[Make list items same form]
    H -->|Yes| J{Pronoun Clarity?}
    J -->|No| K[Clarify pronoun reference]
    J -->|Yes| L[Grammar OK]
```
