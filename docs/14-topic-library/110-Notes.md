...
```mermaid
flowchart TD
    A[Encounter Information] --> B[Take Notes]
    B --> C[Process & Organize]
    C --> D[Review Regularly]
    D --> E[Create Cheat Sheets]
    E --> F[Apply in Practice]
    F --> G[Refine & Update]
    G --> B
```
...
```mermaid
flowchart LR
    subgraph Cornell["Cornell Note Page"]
        direction TB
        Q["Questions & Keywords"]
        N["Main Notes"]
        S["Summary"]
    end
```
