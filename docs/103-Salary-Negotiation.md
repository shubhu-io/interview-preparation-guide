...
permalink: /103-Salary-Negotiation/
nav_exclude: true
search_exclude: true
---
...
```mermaid
flowchart TD
    A[Receive Offer] --> B[Take Time to Review]
    B --> C[Research Market Rates]
    C --> D[Calculate Total Comp]
    D --> E[Develop Counter-Strategy]
    E --> F[Prepare Script]
    F --> G[Schedule Negotiation]
    G --> H[Execute Negotiation]
    H --> I{Outcome}
    I -->|Agreement| J[Get in Writing]
    I -->|Stalemate| K[Evaluate Non-Monetary]
    I -->|Rejected| L[Decide: Accept or Walk]
    J --> M[Accept Offer]
    K --> M
```
...
### Total Compensation Breakdown
```mermaid
pie title Total Compensation Components
    "Base Salary" : 50
    "Equity/RSUs" : 25
    "Annual Bonus" : 12
    "Sign-On Bonus" : 5
