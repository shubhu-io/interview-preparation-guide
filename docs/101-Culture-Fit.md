...
```mermaid
flowchart TD
    A[Self-Assessment] --> B[Identify Core Values]
    B --> C[Define Work Style Preferences]
    C --> D[Company Research]
    D --> E[Values Alignment Map]
    E --> F{Alignment Level}
    F -->|Strong| G[Prepare Interview Stories]
    F -->|Partial| H[Identify Questions to Explore]
    F -->|Weak| I[Consider Other Opportunities]
    G --> J[Interview with Authenticity]
    H --> J
    J --> K{Offer Decision}
    K -->|Accept| L[Onboarding]
    K -->|Decline| M[Continue Search]
```

### Culture Dimensions Wheel
```mermaid
pie title Culture Assessment Dimensions
    "Values Alignment" : 20
    "Work Style" : 15
    "Communication" : 15
    "Decision Making" : 15
