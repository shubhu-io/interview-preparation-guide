...
permalink: /104-Offer-Discussion/
nav_exclude: true
search_exclude: true
---
...
```mermaid
flowchart TD
    A[Receive Offer] --> B[Request Details in Writing]
    B --> C[Calculate Total Comp]
    C --> D[Research Company]
    D --> E[Assess Growth Potential]
    E --> F[Score Each Dimension]
    F --> G[Compare Multiple Offers]
    G --> H{Decision}
    H -->|Best Fit| I[Accept]
    H -->|Needs Improvement| J[Negotiate]
    H -->|Not Right| K[Decline]
    J --> L{Outcome}
    L -->|Improved| I
    L -->|No Change| M[Re-evaluate]
```

### Decision Weighting
```mermaid
pie title Offer Evaluation Weights
    "Compensation" : 28
    "Growth Potential" : 22
    "Company Stability" : 18
