...
tags: [Resume, Screening, Interview Preparation]
date: 2024-01-03
toc: true
---
...
```mermaid
graph TD
    A[Resume Submitted] --> B{ATS Screening}
    B -->|Pass| C[Human Review]
    B -->|Reject| D[Rejection Email]
    
    C --> E{Recruiter Screening}
    E -->|Pass| F[Hiring Manager Review]
    E -->|Reject| D
    
    F --> G{Phone Screen}
    G -->|Pass| H[Interview Process]
    G -->|Reject| D
    ...
    subgraph "ATS Criteria"
        B1[Keyword Match]
        B2[Format Check]
        B3[Basic Qualifications]
        B4[Experience Level]
    end
    
    subgraph "Human Criteria"
        C1[Experience Quality]
        C2[Cultural Fit Signal]
        C3[Growth Trajectory]
        C4[Specific Skills]
    end
```
...
flowchart LR
    ...
    B --> C[Keyword Identification]
    C --> D[Job Description Analysis]
    D --> E[Matching Algorithm]
    E --> F{Score >= Threshold?}
    F -->|Yes| G[Pass to Recruiter]
    F -->|No| H[Reject/Archive]
    
    C --> C1[Hard Skills]
    C --> C2[Soft Skills]
    C --> C3[Certifications]
    C --> C4[Tools/Technologies]
```
...
```mermaid
graph TD
    subgraph "Resume Layout"
        A[Name/Contact - Top] --> B[Summary - Below]
        B --> C[Experience - Main]
        C --> D[Education - Bottom]
        D --> E[Skills - End]
    end
    
    subgraph "Recruiter Eye Pattern"
        F[Glance at Name/Title] --> G[Scan Current Company]
        G --> H[Check Previous Roles]
        H --> I[Glance Education]
        I --> J[Quick Skills Check]
    end
...
```mermaid
flowchart TD
    A[Resume Received] --> B{ATS Check}
    B -->|Keywords Match| C{Format OK?}
    B -->|No Match| D[Auto-Reject]
    
    C -->|Yes| E{Basic Quals Met?}
    C -->|No| F[Manual Review Needed]
    
    E -->|Yes| G[Recruiter Review]
    E -->|No| D
    
    G --> H{6-Second Pass?}
    H -->|Yes| I[Detailed Review]
    H -->|No| D
    
    I --> J{Experience Relevant?}
    J -->|Yes| K[Phone Screen]
    J -->|No| D
    
    K --> L{Cultural Fit?}
    L -->|Yes| M[Interview Process]
    L -->|No| D
```
