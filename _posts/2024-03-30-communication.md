...
```mermaid
graph TD
    A[Sender] -->|Encode message| B[Channel]
    B -->|Transmit| C[Receiver]
    C -->|Decode| D[Understanding]
    D -->|Feedback| A
    E[Noise/Barriers] -.->|Interferes| B
```

### SBI Feedback Model
```mermaid
graph TD
    A[Situation] -->|"When X happened..."| B[Behavior]
    B -->|"You did Y..."| C[Impact]
    C -->|"This caused Z..."| D[Discussion]
    D --> E[Agree on Next Steps]
```
