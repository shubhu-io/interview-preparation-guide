...
categories: Aptitude
tags: [Reading, Comprehension, Interview Preparation]
date: 2024-01-13
toc: true
---
...
```mermaid
graph TD
    A[Reading Comprehension] --> B[Foundational Skills]
    A --> C[Question Types]
    A --> D[Advanced Techniques]
    B --> B1[Active Reading]
    B --> B2[Vocabulary Building]
    B --> B3[Speed Reading]
    B --> B4[Skimming & Scanning]
    C --> C1[Main Idea Questions]
    C --> C2[Inference Questions]
    C --> C3[Detail Questions]
    C --> C4[Tone & Purpose]
    C --> C5[Vocabulary in Context]
    C --> C6[Comparison Questions]
    C --> C7[Structure Questions]
    D --> D1[Critical Analysis]
    D --> D2[Passage Mapping]
    D --> D3[Elimination Strategy]
    D --> D4[Time Management]
    E[Practice] --> E1[Daily Reading]
    E --> E2[Timed Practice]
    E --> E3[Mock Tests]
    E --> E4[Error Analysis]
```
...
```mermaid
flowchart TD
    A[Receive Passage] --> B[Survey Passage - 30 sec]
    B --> C[Scan Questions - 30 sec]
    C --> D[Active Reading - 2-3 min]
    D --> E[Passage Map - 30 sec]
    E --> F{Answer Questions}
    F --> G[Easy Questions First]
    F --> H[Detail Questions - Go Back to Passage]
    F --> I[Inference Questions - Think Beyond Text]
    F --> J[Tone Questions - Analyze Word Choice]
    G --> K[Review Flagged Questions]
    H --> K
    I --> K
    J --> K
    K --> L[Move to Next Passage]
```
...
```mermaid
flowchart TD
    A[Read All Answer Choices] --> B{Is it too extreme?}
    B -->|Yes| C[ELIMINATE]
    B -->|No| D{Is it too narrow?}
    D -->|Yes| C
    D -->|No| E{Is it too broad?}
    E -->|Yes| C
    E -->|No| F{Is it contradicted by passage?}
    F -->|Yes| C
    F -->|No| G{Does it introduce outside info?}
    G -->|Yes| C
    G -->|No| H{Is it supported by passage?}
    H -->|Yes| I[POTENTIAL ANSWER]
    H -->|No| C
```
...
```mermaid
graph LR
    A[Current Speed] --> B[Practice Chunking]
    B --> C[Expand Eye Span]
    C --> D[Reduce Regression]
    D --> E[Use Pointer]
    E --> F[Timed Drills]
    F --> G[Increased Speed]
    G --> H[Monitor Comprehension]
    H -->|Comprehension OK| I[Increase Speed More]
    H -->|Comprehension Drops| J[Slow Down Slightly]
    J --> F
```
