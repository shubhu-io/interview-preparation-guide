...
```mermaid
graph TD
    A[Question] --> B[Choose relevant story]
    B --> C[Set Situation briefly]
    C --> D[Define Task clearly]
    D --> E[Describe Action in detail]
    E --> F[Share Result with metrics]
    F --> G[Add learning/takeaway]
```
...
graph LR
    ...
    B -->|Leadership| C[Story about influence]
    B -->|Teamwork| D[Story about collaboration]
    B -->|Conflict| E[Story about resolution]
    B -->|Failure| F[Story about learning]
    B -->|Initiative| G[Story about proactivity]
    C --> H[STAR Response]
    D --> H
    E --> H
    F --> H
