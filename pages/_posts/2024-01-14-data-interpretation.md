...
categories: Aptitude
tags: [Data, Interpretation, Interview Preparation]
date: 2024-01-14
toc: true
---
...
```mermaid
graph TD
    A[Data Interpretation] --> B[Chart Types]
    A --> C[Calculation Skills]
    A --> D[Question Patterns]
    B --> B1[Bar Graphs]
    B --> B2[Pie Charts]
    B --> B3[Line Graphs]
    B --> B4[Tables]
    ...
    C --> C4[Averages]
    C --> C5[Approximation]
    D --> D1[Direct Questions]
    D --> D2[Comparison Questions]
    D --> D3[Trend Questions]
    D --> D4[Combination Questions]
    D --> D5[Estimation Questions]
    E[Speed Techniques] --> E1[Elimination]
    E --> E2[Approximation]
    E --> E3[Option Analysis]
    E --> E4[Time Management]
```
...
```mermaid
flowchart TD
    A[Receive DI Problem] --> B[Understand Data - 30 sec]
    B --> C[Read Question Carefully]
    C --> D{What type of question?}
    D -->|Direct Reading| E[Locate value in chart]
    D -->|Calculation| F[Extract values, apply formula]
    D -->|Comparison| G[Calculate difference or ratio]
    D -->|Trend| H[Analyze slope/direction]
    D -->|Estimation| I[Use approximation]
    E --> J[Verify answer makes sense]
    F --> J
    G --> J
    H --> J
    I --> J
...
flowchart TD
    ...
    B -->|"X% of Y"| C[Multiply: X/100 × Y]
    B -->|"X is what % of Y"| D[Divide: X/Y × 100]
    B -->|"% increase"| E[Formula: New-Old/Old × 100]
    B -->|"% decrease"| F[Formula: Old-New/Old × 100]
    B -->|Successive changes| G[Formula: A+B+AB/100]
```

### Diagram 3: Chart Selection Guide
```mermaid
flowchart TD
    A[What data do you have?] --> B{Comparing categories?}
    B -->|Yes| C[Bar Graph]
    B -->|No| D{Showing proportions?}
    D -->|Yes| E[Pie Chart]
    D -->|No| F{Trend over time?}
    F -->|Yes| G[Line Graph]
    F -->|No| H{Multiple variables?}
    H -->|Yes| I[Table]
    H -->|No| J[Data in text?]
    J -->|Yes| K[Caselet]
    J -->|No| L[Mixed Charts]
